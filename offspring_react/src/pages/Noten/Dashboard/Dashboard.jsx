...
const Dashboard = () => {
  ...
  // Noten und Ausbildungsdetails laden
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const { faecher, leistungsnachweise } = await fetchAusbildungsDetails(user?.ausbildung?.name);
        setFaecher(faecher);
  ...
  const handleAddGrade = async (newGrade) => {
    try {
      ...
      if (response && response.data) {
        setGrades((prevGrades) => [...prevGrades, response.data]); // State aktualisieren
      }
    } catch (err) {
  ...
  return (
    <Box sx={{ padding: 3 }}>
      ...
      <ButtonGroup variant="contained" sx={{ mb: 3 }}>
        <Button onClick={() => setActiveView("overview")} disabled={activeView === "overview"}>
          Übersicht
        </Button>
        <Button onClick={() => setActiveView("chart")} disabled={activeView === "chart"}>
          Diagramm
        </Button>
        <Button onClick={() => setActiveView("list")} disabled={activeView === "list"}>
          Notenliste
        </Button>
        <Button onClick={() => setActiveView("add")} disabled={activeView === "add"}>
          Note hinzufügen
        </Button>
      </ButtonGroup>
      ...
      {activeView === "list" && <GradeList grades={grades} faecher={faecher} leistungsnachweise={leistungsnachweise} />}
      {activeView === "add" && <AddGradeForm faecher={faecher} leistungsnachweise={leistungsnachweise} onAddGrade={handleAddGrade} />}
    </Box>
  );
};
